...
func Boot(opts ...Option) (*Booted, error) {
	...
	if booted.Config.App.Debug {
		cb, _ := json.Marshal(booted.Config)
		booted.Logger.WithField("config", string(cb)).Info("print config")
	}
	...
	if booted.Config.DB.AutoMigrate {
		booted.Logger.Info("boot db: auto migrate")
		if err = booted.DB.AutoMigrate(&model.User{}, &model.Task{}, &model.TaskGroup{}, &model.Point{}); err != nil {
			return nil, fmt.Errorf("boot db: auto migrate: %w", err)
		}
	}
